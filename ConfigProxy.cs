        ...
        private async Task ProxyAndRewriteResponse(string configUrl, int chatPort, IHttpContext ctx)
        {
            ...
            using (var message = new HttpRequestMessage(HttpMethod.Get, url))
            {
                ...
                try
                {
                    ...
                    {
                        var affinities = (JObject)configObject["chat.affinities"];
                        if ((bool)configObject["chat.affinity.enabled"])
                        {
                            ...
                            riotChatHost = affinities[affinity].ToString();
                        }

                        foreach (var key in affinities.Properties()) // clone to prevent concurrent modification
                        {
                            affinities[key.Name] = "127.0.0.1";
                        }
                    }
